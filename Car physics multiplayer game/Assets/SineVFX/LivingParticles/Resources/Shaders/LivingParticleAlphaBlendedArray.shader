Shader "SineVFX/LivingParticles/LivingParticleAlphaBlendedArray"
{
	...
	SubShader
	{
		...
		void vertexDataFunc( inout appdata_full v, out Input o )
		{
			...
			float4 normalizeResult41;
			for (int w = 0; w < _AffectorCount; w++) {
			if(w == 0){
			DistanceMask45 = distance( appendResult17 , _Affectors[w] );
			normalizeResult41 = normalize( appendResult17 - _Affectors[w] ) * ( clamp( (0.0 + (( 1.0 - distance( appendResult17 , _Affectors[w] ) + ( _Distance - 1.0 ) ) - 0.0) * (1.0 - 0.0) / (_Distance - 0.0)) , 0.0 , 1.0 ) + 0.00001 );
			}else{
			DistanceMask45 = min( DistanceMask45, distance( appendResult17 , _Affectors[w] ) );	
			normalizeResult41 = normalizeResult41 + normalize( appendResult17 - _Affectors[w] ) * ( clamp( (0.0 + (( 1.0 - distance( appendResult17 , _Affectors[w] ) + ( _Distance - 1.0 ) ) - 0.0) * (1.0 - 0.0) / (_Distance - 0.0)) , 0.0 , 1.0 ) + 0.00001 );
			}
			}
			DistanceMask45 = 1.0 - DistanceMask45;
			normalizeResult41 = normalize(normalizeResult41);
