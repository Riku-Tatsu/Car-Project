...
v2f vert (appdata v) {
	...
	#if _BUMPLARGE_ON
		#if _WORLDSPACE_ON
		o.AnimUV2 =  AnimateLargeBump(_BumpMapLarge_ST, worldPos.xz, float2(0.1,0.1) );
		#else
	...
	//Foam
		...
		#if _FOAM_ON
			#if _WORLDSPACE_ON
			o.FoamUV =	TRANSFORM_TEX(worldPos.xz,_FoamTex);
			#else
...
fixed4 frag( v2f i ) : SV_Target
{
	...
	//FOAM ======================================================================
		...
		fixed foamTex = tex2D(_FoamTex,foamUV + (finalBump.xy * 0.05)).r;
		//Final Foam 
		fixed3 foam = 1.0 - saturate(_FoamSize * (sceneZ-DepthUV.z) );
	...
	// NdotL
	fixed NdotL 	= DiffuseTerm(worldN,lightDir);
