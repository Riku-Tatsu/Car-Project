...
fixed4 EdgeLengthBasedTessCull (float3 v0, float3 v1, float3 v2)
{
	fixed4 tess;
	...
	float dist = min(distance(v0, _WorldSpaceCameraPos), min(distance(v1, _WorldSpaceCameraPos), distance(v2, _WorldSpaceCameraPos)));
	return dist < _GrassFadeEnd ? max(tess, 2) : 0;
}
...
HS_CONSTANT_OUTPUT HSConstant(InputPatch<tess_appdata, 3> ip)
{
	...
	fixed4 rTess = EdgeLengthBasedTessCull(ip[0].vertex.xyz, ip[1].vertex.xyz, ip[2].vertex.xyz);
	fixed tess = nextPow2(rTess.w);
...
#if defined(GRASS_FOLLOW_SURFACE_NORMAL)
